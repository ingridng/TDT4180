...
/**
 * Created by ingridng on 05.02.15.
 */
...
public class Avtale extends Application {
    ...
    class SurveyWizard extends Wizard {
        ...
        public SurveyWizard(Stage owner) {
            super(new Formål(),new RomNr(),new Dato(), new Fra(),new Til(),new RepFrekvens(),new Sluttdato(), new Fullført());
            this.owner = owner;
        }
        public void finish() {
            System.out.println(String.format( "Avtale:"));
            System.out.println("Formål: " + SurveyData.formål);
    ...
    class Fra extends WizardPage {
        ...
        Parent getContent() {
            ...
            return VBoxBuilder.create().spacing(5).children(
                    new Label("Oppgi starttidspunkt (hh:mm) ?"),
                    textArea
            ).build();
    ...
    class Til extends WizardPage {
        ...
        Parent getContent() {
            TextArea textArea = TextAreaBuilder.create().wrapText(false).promptText("Sluttidspunkt (hh:mm)").build();
            nesteButton.setDisable(false);
            textArea.textProperty().addListener(new ChangeListener<String>() {
                @Override public void changed(ObservableValue<? extends String> observableValue, String oldValue, String newValue) {
                    nesteButton.setDisable(senere||newValue.length() != 5 || Integer.parseInt(newValue.substring(0, 2)) > 24 || Integer.parseInt(newValue.substring(3, 5)) > 59 || newValue.charAt(2) != ':');
                    SurveyData.til=newValue;
                    if(Integer.parseInt(SurveyData.fra.substring(0,2))==Integer.parseInt(newValue.substring(0, 2))){
                        if(Integer.parseInt(SurveyData.fra.substring(3,5))< Integer.parseInt(newValue.substring(3,5))){
                    senere=true;}}
                        if(Integer.parseInt(SurveyData.fra.substring(0,2))<Integer.parseInt(newValue.substring(0,2))){
                            senere=true;}
                }
            ...
            return VBoxBuilder.create().spacing(5).children(
                    new Label("Oppgi sluttidspunkt (hh:mm) "),
                    textArea
            ).build();
    ...
    class RepFrekvens extends WizardPage {
        ...
        Parent getContent() {
            ...
            textArea.textProperty().addListener(new ChangeListener<String>() {
                @Override public void changed(ObservableValue<? extends String> observableValue, String oldValue, String newValue) {
                    nesteButton.setDisable(newValue.isEmpty()
                            //newValue.charAt(0)!='0'||newValue.charAt(0)!='2'||newValue.charAt(0)!='3'||newValue.charAt(0)!='4'
